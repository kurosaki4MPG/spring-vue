...
class TaskControllerTest {
  ...
  void crudFlowCreatesReadsUpdatesAndDeletesTask() throws Exception {
    ...
    HttpResponse<String> listResponse =
        httpClient.send(get("/api/tasks"), HttpResponse.BodyHandlers.ofString());
    ...
    HttpResponse<String> findResponse =
        httpClient.send(get("/api/tasks/" + createdId), HttpResponse.BodyHandlers.ofString());
    ...
    HttpResponse<String> deleteResponse =
        httpClient.send(delete("/api/tasks/" + createdId), HttpResponse.BodyHandlers.ofString());

    assertThat(deleteResponse.statusCode()).isEqualTo(204);

    HttpResponse<String> deletedFindResponse =
        httpClient.send(get("/api/tasks/" + createdId), HttpResponse.BodyHandlers.ofString());

    assertThat(deletedFindResponse.statusCode()).isEqualTo(404);
    assertThat(deletedFindResponse.body())
        .contains("\"error\":\"Task not found: " + createdId + "\"");
  }
  ...
  void createRejectsBlankTitle() throws Exception {
    ...
    assertThat(response.body())
        .contains("\"error\":\"Validation failed\"", "\"details\":", "\"field\":\"title\"");
  }
  ...
  private HttpRequest delete(String path) {
    return HttpRequest.newBuilder(uri(path)).DELETE().build();
  }
