...
- エラー表示を見出し付きの`role="alert"`へ変更し、スクリーンリーダーにも通知できるようにした。
- HTTPエラーの本文が空の場合、エラー詳細またはHTTPステータスを表示するようにした。
- 通信失敗時は「サーバーに接続できませんでした。再読込してください。」と表示するようにした。
- `App.test.ts`にHTTPステータス表示と通信エラー案内の単体テストを追加した。
- 既存E2Eのタイトル未入力、HTTP 400、初期通信エラーで`.alert--error`の表示とスナップショット取得を確認している。
- 単体テストには画像エビデンスがなく、`role="alert"`・`aria-live`・「エラー」見出しを明示検証するE2Eエビデンスは未取得。
- エラー表示改善後のFrontend単体テスト7件、型チェック、Frontend本番ビルドが成功した。
